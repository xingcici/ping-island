...
- Model-assisted Hook approval configuration, OpenAI-compatible decision calls, Keychain credentials, and 30-day local audit: `PingIsland/Services/Approval/`, `PingIsland/Core/Settings.swift`, `PingIsland/UI/Views/SettingsWindowView.swift`
  - Automated release coverage for model protocol failures, risk policy, duplicate and high-volume Hook concurrency, manual-response races, hint presentation, and full audit export is tracked in `docs/ai-approval-test-matrix.md`
...
- `PingIsland/Services/Window/IDEExtensionInstaller.swift`: installs the VS Code-compatible terminal-focus extension used by Cursor / VS Code / CodeBuddy / Qoder / Qoder CN style IDE hosts (`QoderWork` is hook-only, not an IDE extension host)
...
- If you change model-assisted approval behavior, keep it limited to responsive local/remote Hook approvals, preserve manual-response races and existing explicit auto-approve precedence, never auto-grant session scope, and keep API keys out of defaults, logs, telemetry, diagnostics, and audit records. Risk levels are an independent multi-select for mandatory manual confirmation; evaluating requests stay out of manual-attention UI/sounds, with only the optional lightweight hint allowed. Keep request identity keyed by both session ID and tool-use ID so concurrent Hook callbacks cannot supersede each other; Codex snapshot history and thread-list refreshes must preserve locally pending Hook approval tools and restore the active approval phase. Preserve the session-linked full-context JSON audit export plus `AIApproval` lifecycle logs.
  - Full audit records are persisted incrementally in local SQLite; keep legacy monolithic-file migration off the approval hot path and preserve the existing exported JSON array.
  - Model requests use a 64 KiB layered context budget: provider-maintained `session_summary`, the latest user instruction, a recent user/assistant window, and bounded current-request fields. Keep this model payload compaction separate from the full, unredacted audit context.
  - Obvious destructive version-control operations such as `git reset --hard` have a local high-risk floor so compatible models cannot downgrade them below the user's high-risk confirmation policy.
...
- Release automation:
  ...
  - Fork-only `v*-x` tags use `.github/workflows/fork-release-packages.yml` to publish `-x`-marked ad-hoc signed DMG/ZIP assets without upstream signing secrets; this lane is not notarized and does not publish a Sparkle appcast
