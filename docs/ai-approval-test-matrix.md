...
| Codex 快照竞争 | 5 个并发 Hook 审批入队后，rollout/app-server 快照即使只带部分 `running` 工具，也不能覆盖或丢失本地 `waitingForApproval` 队列 | `testConcurrentCodexHookPermissionsSurviveSnapshotHistoryRefresh` |
