...
| Codex 刷新竞争 | 5 个并发 Hook 审批入队后，rollout/app-server 快照即使只带部分 `running` 工具也不能覆盖队列；每次人工确认后插入 thread-list idle 刷新，剩余审批仍逐条可见 | `testConcurrentCodexHookPermissionsSurviveSnapshotHistoryRefresh`、`testConcurrentHighRiskCodexHooksRemainQueuedAcrossIdleRefreshForSequentialManualReview` |
