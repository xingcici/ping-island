...
The workflow will upload the signed `.dmg`, `.zip`, and zipped Linux `musl` bridge payloads to the matching GitHub Release draft and add a short note that the artifacts were signed and notarized in CI. The Linux bridge artifacts are built with Swift's Static Linux SDK so remote hosts do not need a matching `glibc` version.
